...
namespace ImGuiProfileUtils
{
	...
	void ProfilerGraph::RenderTimings(int graphWidth, int legendWidth, int height, int frameIndexOffset)
	{
		...
		const dm::float2 widgetPos = Vec2(ImGui::GetCursorScreenPos());
		RenderGraph(drawList, widgetPos, dm::float2(graphWidth, height), frameIndexOffset);
		RenderLegend(drawList, widgetPos + dm::float2(graphWidth, 0.0f), dm::float2(legendWidth, height), frameIndexOffset);
	...
	void ProfilerGraph::RenderGraph(ImDrawList* drawList, dm::float2 graphPos, dm::float2 graphSize, size_t frameIndexOffset)
	{
		...
		for (size_t frameNumber = 0; frameNumber < frames.size(); frameNumber++)
		{
			...
			dm::float2 framePos = graphPos + dm::float2(graphSize.x - 1 - frameWidth - (frameWidth + frameSpacing) * frameNumber, graphSize.y - 1);
			if (framePos.x < graphPos.x + 1)
				break;
			dm::float2 taskPos = framePos + dm::float2(0.0f, 0.0f);
			auto& frame = frames[frameIndex];
			for (const auto& task : frame.tasks)
			{
				...
				if (abs(taskEndHeight - taskStartHeight) > heightThreshold)
					Rect(drawList, taskPos + dm::float2(0.0f, -taskStartHeight), taskPos + dm::float2(frameWidth, -taskEndHeight), task.color, true);
			}
		}
	}
	void ProfilerGraph::RenderLegend(ImDrawList* drawList, dm::float2 legendPos, dm::float2 legendSize, size_t frameIndexOffset)
	{
		...
		dm::float2 textMargin = dm::float2(5.0f, -3.0f);
		...
		for (size_t taskIndex = 0; taskIndex < currFrame.tasks.size(); taskIndex++)
		{
			...
			dm::float2 markerLeftRectMin = legendPos + dm::float2(markerLeftRectMargin, legendSize.y);
			dm::float2 markerLeftRectMax = markerLeftRectMin + dm::float2(markerLeftRectWidth, 0.0f);
			markerLeftRectMin.y -= taskStartHeight;
			markerLeftRectMax.y -= taskEndHeight;

			dm::float2 markerRightRectMin = legendPos + dm::float2(markerLeftRectMargin + markerLeftRectWidth + markerMidWidth, legendSize.y - markerRigthRectMargin - (markerRightRectHeight + markerRightRectSpacing) * stat.onScreenIndex);
			dm::float2 markerRightRectMax = markerRightRectMin + dm::float2(markerRightRectWidth, -markerRightRectHeight);
			...
			Text(drawList, markerRightRectMax + textMargin + dm::float2(nameOffset, 0.0f), textColor, (std::string("ms] ") + task.name).c_str());
		}
